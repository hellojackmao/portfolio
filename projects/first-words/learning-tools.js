...
window.FirstWordsTools = (() => {
  ...
  function finishLesson(index, onFinish){
    ...
    const place=({korean:['seoul','Seoul',2,6,7],japanese:['tokyo','Tokyo',4,9,11],cantonese:['hongkong','Hong Kong',4,9,11],'traditional-mandarin':['taipei','Taipei',4,9,11]})[config.track];
    const moment=place?({[place[2]]:['greeting','Greet someone in '+place[1]],[place[3]]:['ordering','Order in '+place[1]],[place[4]]:['directions','Find your way in '+place[1]]})[index]:null;
    const destination=moment?'<section class="destination-next" aria-labelledby="destinationNextTitle"><p class="learning-progress">Destination practice · Optional</p><h4 id="destinationNextTitle">'+moment[1]+'</h4><p>Take the phrase you just learned into a short, three-step activity.</p><a class="btn destination-cta" href="practice.html?place='+place[0]+'&amp;moment='+moment[0]+'">Try it in '+place[1]+' <span aria-hidden="true">→</span></a></section>':'';
    config.openSheet(moment?'Lesson complete':'A good place to pause','<div class="learning-finish"><p class="learning-progress">'+esc(config.lessons[index].title)+' · Complete</p><h3 tabindex="-1" id="finishTitle">'+(moment?'Ready to use it?':'Enough for today, if you like.')+'</h3><p>Your progress is saved. '+(moment?'Try a moment in the city, or stop here and return when you like.':'You can stop here and return when you are ready.')+'</p>'+destination+'<div class="reference-actions"><button type="button" class="btn '+(moment?'ghost':'')+'" id="finishNow">Finish for now</button>'+(next>=0?'<button type="button" class="btn ghost" id="continueLesson">Next: '+esc(config.lessons[next].title)+'</button>':'')+'</div></div>');
